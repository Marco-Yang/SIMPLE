...
@TaskRegistry.register("g1_wholebody_bend_pick_mp")
class G1WholebodyBendPickMP(Task):
    uid: str = "g1_wholebody_bend_pick_mp"
    label: str = "G1 Wholebody Bend Pick MP"
    description: str = (
        "A task where the G1 Wholebody robot must bend and pick up an object."
    )
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.6, -0.0, 0.38], high=[-0.66, 0.0, 0.38]),
            target_region=Box(low=[-0.29, -0.08], high=[-0.32, -0.04]),
        ...
        lighting=LightingDRCfg(
            ...
            light_intensity=Box(low=1e4, high=2e4),
